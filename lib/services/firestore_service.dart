...
class FirestoreService {
  ...
  */
  ...
  Future<void> saveAllSurveyResponse(List<SurveyResponse> responses) {
    ...
    for (final res in responses) {
      DocumentReference _docRef =
          _db.collection(DotEnv().env['_survey_response_path']).doc();
  ...
  Stream<List<QuestionItem>> getSurveyQuestions() {
    ...
    */
    return _db
        .collection(DotEnv().env['_survey_questions_path'])
        // .get() //future
        .snapshots()
        .map((snapshots) => snapshots.docs
  ...
  Future<void> deleteHrMessage(id) {
    return _db.collection(DotEnv().env['_hrMessages_path']).doc(id).delete();
  }

  // Save new report a bug -message
  Future<void> saveReportBugMsg(ReportMsgModel response) {
    return _db
        .collection(DotEnv().env['_reportBug_path'])
        .doc()
        .set(response.toMap());
  }
}
